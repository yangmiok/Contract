...
contract Pensions is IPensions {
    ...
    constructor(address _administrators) {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("Pensions")),
                keccak256(bytes('1')),
                address(this)
            )
        );

        WITHDRAWAL_TYPEHASH = keccak256("Withdrawal(uint256 txId,address account,uint256 amount,uint256 deadline)");
    }
    ...
    function withdrawalWithPermit(
        ...
        returns (bool success)
    {
        require(block.timestamp <= _deadline, "Pensions: Signatures beyond the validity period");
        require(!isExecuted[_txId], "Pensions: Orders have been executed");
        ...
        require(verify(vc, v, r, s), "Pensions: Authentication failure");
    ...
    function verify(ValidationConditions memory vc,uint8 v,bytes32 r,bytes32 s) internal view returns (bool) {
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                '\x19\x01',
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(WITHDRAWAL_TYPEHASH, vc.txId, vc.account, vc.amount,vc.deadline))
            )
        );
